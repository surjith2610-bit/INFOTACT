...
def process_transaction(tx: dict, driver):
    ...
    logger.info(f"[KAFKA] Transaction received: {sender} -> {receiver} (${amount:,.2f})")

    # 1. Update Neo4j graph synchronously
    cypher_write = """
    ...
    with driver.session(database=NEO4J_DB) as session:
        session.run(cypher_write, {"sender": sender, "receiver": receiver, "txId": tx_id, "amount": amount, "timestamp": timestamp})
